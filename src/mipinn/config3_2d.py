...
# 真实数据文件路径
REAL_DATA_FILE = 'data4/t=0.00s.txt'  # 真实实验数据文件（使用data4文件夹，从0秒开始）
...
# 时间参数（瞬态模拟）
TIME_START = 0.0  # 开始时间（0.0s，基于data2数据范围0-7s）
TIME_END = 7.0   # 结束时间（7.0s，基于gemini.md指令要求）
TIME_STEP = 0.01  # 时间步长（0.01秒，共701步）
N_TIME_STEPS = 701  # 时间步骤数量（701步，0-7秒，每0.01秒一步）

# 时间分段参数（t1:真实数据范围，t2:Mamba约束范围）
T1_START = 0.0   # 真实数据开始时间（t1范围：0-2.5s）
T1_END = 2.5    # 真实数据结束时间（t1范围：0-2.5s）
T2_START = 2.5  # Mamba约束开始时间（t2范围：2.5-7s）
T2_END = 7.0    # Mamba约束结束时间（t2范围：2.5-7s）
...
# 网格参数定义（m 单位，国际单位制）
# 基于实际数据范围：x=9975-17975mm，y=1975-5975mm（data4数据集）
X_MIN = 10.5  # x方向最小值（从10.5m开始而不是9.975m）
X_MAX = 17.975  # x方向最大值（17975mm -> 17.975m）
Y_MIN = 1.975  # y方向最小值（1975mm -> 1.975m）
...
# 归一化参数（基于实际数据范围，m单位）
norm_params = {
    'x_min': X_MIN,      # m，x方向最小值（9.975m）
    'x_max': X_MAX,      # m，x方向最大值（17.975m）
    'y_min': Y_MIN,      # m，y方向最小值（1.975m）
    'y_max': Y_MAX,      # m，y方向最大值（5.975m）
    'z_min': PLANE_SIZE_Z,
    'z_max': PLANE_SIZE_Z,
    'u_min': 0.12,       # m/s，基于三圆柱绕流实际数据u范围约0.12-1.70
    'u_max': 1.70,       # m/s，u方向最大值
    'v_min': -0.60,      # m/s，基于三圆柱绕流实际数据v范围约-0.60-0.65
    'v_max': 0.65,       # m/s，v方向最大值
    't_min': TIME_START,
    't_max': 10.0        # m/s，时间最大值改为10.0秒
}
